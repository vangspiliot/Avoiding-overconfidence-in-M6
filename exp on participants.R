...
for (sid_c in 1:length(unique(submission_data$Team))){
  ...
  for (pid_c in c(1:12)){
    ...
    if (nrow(submission_tmp)>0){ 
      ...
      #Adjust submission
      submission_tmp$Rank1 = submission_tmp$Rank5 <- (submission_tmp$Rank1 + submission_tmp$Rank5)/2
      submission_tmp$Rank2 = submission_tmp$Rank4 <- (submission_tmp$Rank2 + submission_tmp$Rank4)/2
      submission_tmp$Decision <- submission_tmp$Rank5/sum(submission_tmp$Rank5)
