...
#Identify Fridays
sub_period_end <- as.Date(c(seq.Date(as.Date("2021-04-02"),as.Date("2022-03-04"),28),
                            "2022-04-01", "2022-04-29","2022-05-27", 
...
for (aid in unique(asset_data$symbol)){
  ...
  xrange <- c(13:24) #Repeat evaluation for 48 consecutive weeks
...
#save.image("results_fromR_m6.Rdata")
load("results_fromR_m6.Rdata")
...
setEPS()
postscript("rps_methods_m6.eps", width=6, height=4.5)
...
for (mid in mname){
  ...
  for (xstart in xrange){
    ...
    ret_m <- unlist(lapply(c(2:length(colnames(ret_tot))), function(x)log(1+sum(ret_m[,x]*ret_m$weight))))
...
setEPS()
postscript("IR_RPS_m6.eps", width=9, height=6)
par(mfrow=c(1,1))
plot(inv_results$RPS,inv_results$IR, ylim=c(-1,2.5),
     ylab = "IR", xlab = "RPS")
